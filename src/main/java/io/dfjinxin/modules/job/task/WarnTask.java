...
public class WarnTask implements ITask {
	...
	public void run(String params) throws Exception {
		...
		//预警是否过期
//		boolean isInDate =new DateTime(new Date()).between(new DateTime(pe.getBeginDate()),new DateTime(pe.getEndDate()));
//		if("1".equals(pe.getDelFlag())||!isInDate){
//			return;
//		}
		...
		if("1".equals(pe.getEwarnTerm())){
			BigDecimal price0=wys.get(0).getPriToday();
			BigDecimal price1=wys.get(1).getPriToday();
			BigDecimal b1=compareResult(price0,price1);
			pee.setPriRange(b1);
			//预警级别//R_W:红   O_W:橙     Y_W:黄     G_W:绿
			pee.setEwarnLevel(ewarnLevelResult(b1.abs(),pe));
		}
		pee.setPriValue(wys.get(0).getPriToday());
		if("2".equals(pe.getEwarnTerm())){
			BigDecimal price0=wys.get(0).getPriToday();
			BigDecimal price1=wys.get(1).getPriToday();
			BigDecimal price2=wys.get(2).getPriToday();
			BigDecimal price3=wys.get(3).getPriToday();
	...
	private String ewarnLevelResult(BigDecimal b1,PssEwarnConfEntity pe){
		//红色预警
		if(b1.compareTo(pe.getEwarnLlmtRed())>=0&&b1.compareTo(pe.getEwarnUlmtRed())<=0) return "R_W";
		//橙色预警
		else if(b1.compareTo(pe.getEwarnLlmtOrange())>=0&&b1.compareTo(pe.getEwarnUlmtOrange())<=0) return"O_W";
		//黄色预警
		else if(b1.compareTo(pe.getEwarnLlmtYellow())>=0&&b1.compareTo(pe.getEwarnUlmtYellow())<=0) return "Y_W";
		//绿色预警
		else if(b1.compareTo(pe.getEwarnLlmtGreen())>=0&&b1.compareTo(pe.getEwarnUlmtGreen())<=0) return "G_W";
		else return null;
	}
}
